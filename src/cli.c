...
void check_file_access(char *_file, bool inputFile)
{
  if (access(_file, F_OK) != 0)
  {
    if (inputFile)
    {
      add_to_errors(create_error_without_linecolumn(FILE_ACCESS, "Missing access to input file", true));
    }
    else
    {
      add_to_errors(create_error_without_linecolumn(FILE_ACCESS, "File for output does not exist, creating...",
                                                    false));
    }
  }
}
...
void check_file_missing(char *_file)
{
  ...
  if (_file != NULL)
  {
    ...
    if (_fp == NULL)
    {
      add_to_errors(create_error_without_linecolumn(FILE_ACCESS, "Cannot create/open output file", true));
    }
    fclose(_fp);
  }
  else
...
void proc_cli(int argc, char *argv[])
{
  ...
  {
    add_to_errors(create_error_without_linecolumn(FILE_ACCESS, "Input filename is empty.", true));
  }
  else
  {
